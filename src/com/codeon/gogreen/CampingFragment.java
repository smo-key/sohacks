...
public class CampingFragment extends Fragment implements
	...
	public class PlacesTask extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String[] paramArrayOfString) {
			...
			request = httpGet("https://trailapi-trailapi.p.mashape.com/"
					...
					+ mCurrentLocation.getLongitude() + "&radius=50"
					+ "&limit=15");
			...
			try {
				...
				for (int i = 0; i < results.length(); i++) {
					...
					double distance = distance(mCurrentLocation.getLatitude(),
							mCurrentLocation.getLongitude(), lat, lng);

					parkslist.add(new Park(j.getString("city") + ", "
							+ j.getString("state"), j.getString("name"),
							distance));
	...
	public String httpGet(String s) {
		...
		HttpClient httpclient = new DefaultHttpClient(); // create new
															// httpClient
		HttpGet httpGet = new HttpGet(url); // create new httpGet object
		httpGet.setHeader("X-Mashape-Authorization",
				"sP9Qf5DbYom8Zqvb49OSFVb2iAifJKuS");

		try {
			org.apache.http.HttpResponse response = httpclient.execute(httpGet); // execute
																					// httpGet
